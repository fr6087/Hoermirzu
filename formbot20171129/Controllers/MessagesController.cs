...
{
    /// <summary>
    /// central entry point of the bot. This class receives all messages by the user in the post()-method, where
            ...
            return Chain.From(() => FormDialog.FromForm(ESF2CompanyDetailsForm.BuildLocalizedForm))
                .Do(async (context, order) =>
                {
                    try
                    {
                        ...
                        await Conversation.SendAsync(new Activity(Resource1.AfterOrder), MakeRootDialog);
                    }
                    catch (FormCanceledException<ESF2CompanyDetailsForm> e)
                    {
                        ...
                        {
                            reply = Resource1.StopUnexpected+ $"{e.Message}";
                        }
                        else
                        {
                            reply = Resource1.TryLater + $"{e.Message}";
                        }
            ...
            return Chain.From(() => FormDialog.FromForm(ESF2CompanyDetailsForm.BuildJsonForm))
                .Do(async (context, order) =>
                {
                    try
                    {
                        ...
                        var activity = new Activity("Processed your order!");
                        activity.Text = "berechnete formular";
                        activity.Name = "LuisDialogMessage";
                        activity.Type = "message";
                        activity.Code = "Form";*/
                        await context.SayAsync(text: Resource1.Processing, speak: Resource1.Processing);
                    ...
                    catch (FormCanceledException<JObject> e)
                    {
                        ...
                        if (e.InnerException == null)
                        {
                            reply = Resource1.StopUnexpected+$"{e.Last}";
                        }
                        else
                        {
                            reply = Resource1.DialogRetry;
                        }
            ...
            if (activity != null)
            {
                ...
                switch (activity.GetActivityType())
                {
                    case ActivityTypes.Message:
                        ...
                        if(type.Equals("LuisDialogMessage"))
                            await Conversation.SendAsync(activity, () => new BasicLuisDialog());
                        else
                            // await Conversation.SendAsync(activity, MakeJsonRootDialog);
                            // 
                            await Conversation.SendAsync(activity, ()=> new RootDialog(MakeJsonRootDialog));
                        break;
